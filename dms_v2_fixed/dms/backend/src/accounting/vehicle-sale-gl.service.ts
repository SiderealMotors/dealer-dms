...
export class VehicleSaleGlService {
  ...
  async postVehicleSaleJournals(
    ...
    if (revenueCents > 0n && !revenueEntryId) {
      ...
      const amt = new Prisma.Decimal(centsToDecimalString(revenueCents));
      ...
      const hstAmt = new Prisma.Decimal(centsToDecimalString(hstCents));
      const totalWithHst = new Prisma.Decimal(centsToDecimalString(revenueCents + hstCents));
    ...
    if (costCents > 0n && !cogsEntryId) {
      ...
      const amt = new Prisma.Decimal(centsToDecimalString(costCents));
