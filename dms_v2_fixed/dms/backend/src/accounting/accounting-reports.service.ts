...
export class AccountingReportsService {
  ...
  async incomeStatement(query: IncomeStatementQueryDto) {
    ...
    for (const g of grouped) {
      ...
      const dr = g._sum.debitAmount ?? new Prisma.Decimal(0);
      const cr = g._sum.creditAmount ?? new Prisma.Decimal(0);
  ...
  async monthlySummaries(query: AccountingDateRangeQueryDto) {
    ...
    const glByMonth = await this.prisma.$queryRaw<
      { month: Date; net_revenue: Prisma.Decimal; net_expense: Prisma.Decimal }[]
    >(
      Prisma.sql`
        SELECT
