...
async function main() {
  ...
  await prisma.dealerSettings.upsert({
    ...
    create: {
      ...
      province: 'ON',
      hstRate: new Prisma.Decimal(0.13),
      defaultOmvicFee: new Prisma.Decimal(75.00),
    },
  });

  // ── Full Ontario used-car dealer chart of accounts ────────────────────────
  const coa: Array<{ code: string; name: string; type: AccountType; normalBalance: NormalBalance; description?: string; parentCode?: string }> = [
    // ASSETS
    { code: '1000', name: 'Cash – operating account', type: AccountType.ASSET, normalBalance: NormalBalance.DEBIT, description: 'Main chequing account' },
    { code: '1010', name: 'Cash – savings', type: AccountType.ASSET, normalBalance: NormalBalance.DEBIT },
    { code: '1100', name: 'Accounts receivable', type: AccountType.ASSET, normalBalance: NormalBalance.DEBIT, description: 'Amounts owed by buyers' },
    { code: '1150', name: 'A/R – financed deals', type: AccountType.ASSET, normalBalance: NormalBalance.DEBIT, description: 'Finance house receivable' },
    { code: '1200', name: 'Vehicle inventory', type: AccountType.ASSET, normalBalance: NormalBalance.DEBIT, description: 'Cost basis of vehicles on lot' },
    ...
    { code: '2400', name: 'Deposits held', type: AccountType.LIABILITY, normalBalance: NormalBalance.CREDIT, description: 'Customer deposits not yet applied' },
    { code: '2500', name: 'Floorplan loan payable', type: AccountType.LIABILITY, normalBalance: NormalBalance.CREDIT, description: 'Outstanding floorplan financing' },
    ...
    // REVENUE
    { code: '4000', name: 'Vehicle sales revenue', type: AccountType.REVENUE, normalBalance: NormalBalance.CREDIT, description: 'Pre-tax selling price proceeds' },
    ...
    // COGS
    { code: '5000', name: 'Cost of goods sold – vehicles', type: AccountType.EXPENSE, normalBalance: NormalBalance.DEBIT, description: 'Acquisition cost incl. purchase HST' },
    ...
    { code: '7300', name: 'Professional fees (accounting/legal)', type: AccountType.EXPENSE, normalBalance: NormalBalance.DEBIT },
  ...
  await upsertVehicleByVin('2T1BURHE0JC123456', {
    ...
    salesPersonId: salesB.id, salesPersonName: salesB.fullName,
  });

  await upsertVehicleByVin('5NPE24AF8FH123456', {
    datePurchased: new Date('2025-02-20'),
  ...
  await upsertVehicleByVin('1N4AL3AP7JC123456', {
    datePurchased: new Date('2025-03-01'),
    vin: '1N4AL3AP7JC123456',
    year: 2021, make: 'Nissan', model: 'Altima', trim: 'S', colour: 'Gun Metallic',
    odometer: 39000, purchasePrice: 17800, safetyEstimate: 400, safetyCost: 320,
    floorplanInterestCost: 150, gas: 55, warrantyCost: 350,
    sellingPrice: 21500, status: VehicleStatus.PENDING,
    salesPersonId: salesB.id, salesPersonName: salesB.fullName,
  });
  ...
  if (await prisma.expense.count() === 0) {
    ...
    const cashAcct = await prisma.glAccount.findFirst({ where: { code: '1000' } });
    const insAcct = await prisma.glAccount.findFirst({ where: { code: '6400' } });

    await prisma.expense.createMany({
      data: [
        { id: 'exp1', date: new Date('2025-03-01'), category: 'RENT', vendor: 'Landlord Inc', description: 'March lot rent', amountPreTax: new Prisma.Decimal(2500), hstAmount: new Prisma.Decimal(325), totalAmount: new Prisma.Decimal(2825), glAccountId: rentAcct?.id },
        { id: 'exp2', date: new Date('2025-03-05'), category: 'INSURANCE', vendor: 'Intact Insurance', description: 'Dealer liability insurance', amountPreTax: new Prisma.Decimal(850), hstAmount: new Prisma.Decimal(0), totalAmount: new Prisma.Decimal(850), glAccountId: insAcct?.id },
        { id: 'exp3', date: new Date('2025-03-10'), category: 'ADVERTISING', vendor: 'AutoTrader', description: 'AutoTrader listing subscription', amountPreTax: new Prisma.Decimal(399), hstAmount: new Prisma.Decimal(51.87), totalAmount: new Prisma.Decimal(450.87) },
      ],
    });
  }
